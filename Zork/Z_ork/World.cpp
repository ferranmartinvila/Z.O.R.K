...
bool world::apply_order(vector<string>& instruction){
	...
	if (me->object_focused != -1){
		...
		//open instruction
		if ((instruction.buffer[0] == "open" && instruction.get_size() == 3 && instruction.buffer[2] == "door")){ me->apply_open_door_instruction(instruction); reader++; }
		//close instruction
		else if ((instruction.buffer[0] == "close" && instruction.get_size() == 3 && instruction.buffer[2] == "door")){ me->apply_close_door_instruction(instruction); reader++; }
	}
